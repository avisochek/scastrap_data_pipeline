...
def get_street_name(issue):
    base_url="http://localhost:80/reverse"
    params={
        "format":"json",
        "lat":str(issue["lat"]),
        "lon":str(issue["lng"])}
...
def get_segment_length(geojson,city):
    ...
    while len(data)>1:
        ...
        n_in_bounds=bounds_polygon.contains(Point(last_node[0],last_node[1]))
        n_in_bounds+=bounds_polygon.contains(Point(current_node[0],current_node[1]))
        if n_in_bounds > 0:
...
def get_street_length(street_name,city):
    base_url="http://localhost:80/search"
    params={
        "format":"json",
        "street":street_name,
        "polygon_geojson":1,
